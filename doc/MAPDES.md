# MAPDES macro

This macro maps the Data Facilities Area

## Syntax

``` hlasm
&LABEL   MAPDES &DSECT=YES,            * YES or NO                     *
               &CB=,                   * Control block to generate     *
               &PRFX=                  * Prefix to use
```

## DSECT

- If `YES` (or omitted) indicates to generate the block as a DSECT.
- if `NO` specifies to generate the block as an embedded area.

## CB

Specify the short name (i.e. without its three-character prefix) of the control block when embedding one with DESCT=NO.

Supported values:
- DESB   - DirEntry Services Buffer header
- DESD   - DirEntry Services member Data descriptor
- DESL   - DirEntry Services name List
- DESN   - DirEntry Services Name record
- DESP   - DirEntry Services Parmlist
- DESR   - DirEntry Services Reason codes
- DESRCS - DirEntry Services Return CodeS
- DESX   - DirEntry Services eXit plist
- DST    - Direntry Services Screen Table

## PRFX

Specify a value to set a prefix when embedding the control block with DESCT=NO.
Ignored for DSECT=YES.

## Macro code

The [MAPDES macro](../bxamac/MAPDES.mac) is in the BXAmac folder.

=======

(C) Copyright 1999-2026 Abe Kornelis. All rights reserved.