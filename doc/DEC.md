# DEC macro

Decrement a register

## Syntax

``` hlasm
&LABEL   DEC   &REG,                   * Register to be decremented    *
               &COUNT                  * Amount to decrement or (reg)
```

## REG

specifies the register to be decremented

## COUNT

specifies the amount by which the register is to be decremented.
May be specified as:
- Literal or symbolic value: must evaluate to a value between 0 and 4095 inclusive.
- (reg): must specify a valid register, 0 not allowed
- (value,reg): both of the above apply
- nothing: defaults to 1

## Macro code

The [DEC macro](../bxamac/DEC.mac) is in the BXAmac folder.

=======

(C) Copyright 1999-2026 Abe Kornelis. All rights reserved.